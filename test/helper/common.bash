...
setup_test_env() {
  ...
  export STUB_CODEX_WORKSPACE_SNAPSHOT=""
  export STUB_CODEX_WORKFLOW_STATE_SNAPSHOT=""
...
run_wrapper() {
  run env \
    ...
    STUB_CODEX_WORKSPACE_SNAPSHOT="$STUB_CODEX_WORKSPACE_SNAPSHOT" \
    STUB_CODEX_WORKFLOW_STATE_SNAPSHOT="$STUB_CODEX_WORKFLOW_STATE_SNAPSHOT" \
...
run_wrapper_with_input() {
  ...
  cmd="$(quote_cmd env \
    ...
    CODEX_WRAPPER_REAL_CODEX="$BATS_TEST_DIRNAME/stubs/codex" \
    "$WRAPPER_RUNNER" "$@")"
  run bash -lc "printf '%s' $(printf '%q' "$input") | $cmd"
}
